...
public class CommandRepositoryImpl implements Repositorio<Command> {
    ...
    public void save(Command command) {
        ...
        if (command.getId() != null && command.getId() > 0) {
            ...
            sql = "INSERT INTO commands(name, description, example, category_id) VALUES(?,?,?,?)";
        }
        ...
        try (PreparedStatement stmt = getConnection().prepareStatement(sql)) {
            ...
            //TODO: aca voy agregar por fecha tengo que implementar un if cuando lo agrege
            //if(command.getId() != null & command.getId() > 0) {
            //    stmt.setLong(4, command.getId());
            //} else {
            //    stmt.setDate(4, new Date(command.getDateRegister().getTime()));
            //}
            stmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
